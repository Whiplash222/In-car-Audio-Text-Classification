...
## 操作说明
通过运行代码文件[pipeline.py](https://github.com/Whiplash222/In-car-Audio-Text-Classification/blob/main/pipeline.py)，未落域文本分类结果会输出为*classification_results.xlsx*，其中每一行将包含一条未落域的文本和它对应的分类结果。（具体分类方法流程及代码详见下一节*分类方法与代码说明*中的具体说明。）可通过将输入数据文档*top1000new.xlsx*替换为其它待分类文本数据文件，来获得其它自定义数据的分类结果。
